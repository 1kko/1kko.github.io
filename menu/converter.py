...
def fetch_attachments():
    ...
    emails = g.label('Ahnapp').mail(prefetch=True, after=After, unread=True)
...
def convert(filename):
    ...
    for plusMenuItem in ws.merged_cells.ranges:

        cell = plusMenuItem.coord.split(":")[0]
    ...
    for time in ['07:30', '11:30', '17:30']:
        ...
        for i in range(0, len(plusMenuCells)):
            ...
            plusStart = int(plusMenuCells[i].coord.split(":")[0].replace("A", ""))
            plusEnd = int(plusMenuCells[i].coord.split(":")[1].replace("B", ""))
    ...
    for mcell in ws.merged_cells:
        cell = re.findall(r"[^\W\d_]+|\d+", mcell.coord)
    ...
    outputfilename = str(filename_date_start.strftime('%Y%m%d')) + "-" + str(
        filename_date_end.strftime('%Y%m%d')) + ".csv"
    ...
    for col in ['C', 'D', 'E', 'F', 'G']:
        ...
        for time in ['07:30', '11:30', '17:30']:
            ...
            for i in range(0, len(meal_category)):
                ...
                description += data_merger(ws, col, meal_category[i]['row']['start'], meal_category[i]['row']['end'],
                                           header=meal_category[i]['data']) + "\n"
