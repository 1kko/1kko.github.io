var Yl = {
 ...
 ,nMaxWidth : window.innerWidth-60 //최고 넓이
 ,nWidth : 0 //사다리 넓이
 ,nHeight : window.innerHeight-400 //사다리 높이
 ...
 ,init : function(){
  ...
  for(var i=0; i<this.nNum; i++){
   ...
   this.arrBotDiv[i].style.top = window.innerHeight-280+"px";
 ...
 ,create : function(){ //사다리 생성
  ...
  for(var i=0; i<this.nNum; i++){
   ...
   this.arrGoDiv[i].innerHTML = '<input type="button" class="btn btn-xs btn-primary" value="GO" onClick="Yl.start('+i+')">';
   ...
   if( nEy<this.nHeight+100 ){
    ...
    for(var i=0; i<this.arrVerDiv.length; i++){
     if(nEx==parseInt(this.arrVerDiv[i].style.left)){      
      ...
      this.arrGoDiv[pno].innerHTML = '<input type="button" class="btn btn-xs btn-success" value="확인" onClick="Yl.start('+pno+')">';
