...
implements MapView.OpenAPIKeyAuthenticationResultListener, MapView.MapViewEventListener, MapView.CurrentLocationEventListener, MapView.POIItemEventListener, MapReverseGeoCoder.ReverseGeoCodingResultListener {
	...
	private String dinner_room_name;
	private String dinner_room_location;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mapView.setMapType(MapView.MapType.Standard); //mapView 기본형


		//Daum mobile map api key authentication
		mapView.setDaumMapApiKey("946188d744f93b6c5a3615ad0b5857d23dc00c7d");
		mapView.setOpenAPIKeyAuthenticationResultListener(this);
		mapView.setPOIItemEventListener(this);
		//parent activity에서 야근식당 이름,경도,위도 받아오기 
		Intent intent = getIntent();
		dinner_room_name = intent.getExtras().getString("dinner_room_name");
		dinner_room_location = intent.getExtras().getString("dinner_room_location");
		longitude = intent.getExtras().getDouble("longitude");
		latitude = intent.getExtras().getDouble("latitude");
		mapView.setCurrentLocationTrackingMode(MapView.CurrentLocationTrackingMode.TrackingModeOnWithoutHeading);

		MapPOIItem poiDinnerRoomItem = new MapPOIItem();
		poiDinnerRoomItem.setItemName(dinner_room_name);
		poiDinnerRoomItem.setUserObject(String.format("item%d", 2));
		poiDinnerRoomItem.setMapPoint(MapPoint.mapPointWithGeoCoord(latitude,longitude));
		poiDinnerRoomItem.setMarkerType(MapPOIItem.MarkerType.CustomImage);
		poiDinnerRoomItem.setShowAnimationType(MapPOIItem.ShowAnimationType.SpringFromGround);
		poiDinnerRoomItem.setCustomImageResourceId(R.drawable.custom_poi_marker);
		poiDinnerRoomItem.setCustomImageAnchorPointOffset(new MapPOIItem.ImageOffset(50,0));
		poiDinnerRoomItem.setShowCalloutBalloonOnTouch(true);
		mapView.addPOIItem(poiDinnerRoomItem);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if(keyCode==KeyEvent.KEYCODE_BACK ){
		    Log.d("Finish DaumMapActivity", "Finish DaumMapActivity");
			finish();
		}
	...
	public void onPOIItemSelected(MapView mapView, MapPOIItem poiItem) {
		...
		AlertDialog.Builder alertDialog = new AlertDialog.Builder(this);
		alertDialog.setTitle(dinner_room_name);
		alertDialog.setMessage(dinner_room_location);
		alertDialog.setPositiveButton("OK", null);
		alertDialog.show();
